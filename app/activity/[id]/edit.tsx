...
export default function EditActivityScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setCats(catRows);
      const rows = await db.select().from(activities).where(eq(activities.id, Number(id)));
      if (rows[0]) {
        setName(rows[0].name);
        setDate(rows[0].date);
  ...
  const handleSave = async () => {
    ...
    await db.update(activities).set({
      name: name.trim(), date, durationMinutes: Number(duration) || 0,
      count: Number(count) || 1, notes: notes.trim() || null,
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={{ backgroundColor: theme.background }} contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.catGrid}>
          {cats.map(c => (
            <TouchableOpacity key={c.id} onPress={() => setSelectedCat(c.id)} style={[styles.catItem, { borderColor: selectedCat === c.id ? c.color : 'transparent', borderWidth: 2 }]}>
        ...
        <TouchableOpacity style={[styles.deleteBtn, { borderColor: theme.danger }]} onPress={handleDelete}>
          <Text style={[styles.deleteBtnText, { color: theme.danger }]}>Delete Activity</Text>
        </TouchableOpacity>
      </ScrollView>
