...
export default function EditTripScreen() {
  ...
  useEffect(() => {
    db.select().from(trips).where(eq(trips.id, Number(id))).then(rows => {
      if (rows[0]) {
        setName(rows[0].name);
  ...
  const handleSave = async () => {
    if (!validate()) return;
    setLoading(true);
    await db.update(trips).set({ name: name.trim(), destination: destination.trim(), startDate, endDate, notes: notes.trim() || null }).where(eq(trips.id, Number(id)));
    setLoading(false);
    router.back();
  };
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={{ backgroundColor: theme.background }} contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <TouchableOpacity style={[styles.btn, { backgroundColor: theme.primary }, loading && styles.btnDisabled]} onPress={handleSave} disabled={loading} accessibilityRole="button" accessibilityLabel="Save changes">
