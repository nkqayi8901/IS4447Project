...
export default function TripDetailScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      const load = async () => {
        const tripRows = await db.select().from(trips).where(eq(trips.id, Number(id)));
        ...
        const acts = await db.select().from(activities).where(eq(activities.tripId, Number(id)));
        ...
        const w = await fetchWeather(tripRows[0].destination.split(',')[0].trim());
        setWeather(w);
      };
      void load();
    }, [id])
  ...
  const handleDelete = () => {
    Alert.alert('Delete Trip', 'This will also delete all activities in this trip. Continue?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete', style: 'destructive',
        onPress: async () => {
          await db.delete(activities).where(eq(activities.tripId, Number(id)));
  ...
  const handleExport = async () => {
    if (!trip) return;
    await exportActivitiesToCSV(trip.name, actList.map(a => ({
      name: a.name, date: a.date, durationMinutes: a.durationMinutes,
      count: a.count, notes: a.notes, category: a.categoryName,
    })));
  };
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <ScrollView stickyHeaderIndices={[1]} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.headerTop}>
            <View style={{ flex: 1 }}>
              ...
              <View style={styles.row}>
                ...
                <Text style={[styles.dest, { color: theme.textSecondary }]}>{trip.destination}</Text>
              </View>
            ...
            <View style={styles.actions}>
              <TouchableOpacity onPress={() => router.push(`/trip/${id}/edit` as any)} style={[styles.iconBtn, { backgroundColor: theme.primary + '20' }]}>
                <Ionicons name="pencil-outline" size={18} color={theme.primary} />
              </TouchableOpacity>
              <TouchableOpacity onPress={handleExport} style={[styles.iconBtn, { backgroundColor: theme.success + '20' }]}>
                <Ionicons name="download-outline" size={18} color={theme.success} />
              </TouchableOpacity>
              <TouchableOpacity onPress={handleDelete} style={[styles.iconBtn, { backgroundColor: theme.danger + '20' }]}>
                <Ionicons name="trash-outline" size={18} color={theme.danger} />
              </TouchableOpacity>
            </View>
          </View>
          {trip.notes ? <Text style={[styles.notes, { color: theme.textSecondary }]}>{trip.notes}</Text> : null}
          {weather ? (
        ...
        <View style={[styles.filterBar, { backgroundColor: theme.background }]}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.chips}>
            {(['all', 'today', 'week'] as FilterPeriod[]).map(p => (
              <TouchableOpacity
                key={p}
                style={[styles.chip, { borderColor: period === p ? theme.primary : theme.border, backgroundColor: period === p ? theme.primary : theme.card }]}
                onPress={() => setPeriod(p)}
              >
                <Text style={[styles.chipText, { color: period === p ? '#fff' : theme.textSecondary }]}>{p.charAt(0).toUpperCase() + p.slice(1)}</Text>
              </TouchableOpacity>
            ))}
            <TouchableOpacity
              style={[styles.chip, { borderColor: selectedCat === null ? theme.border : theme.primary, backgroundColor: selectedCat === null ? theme.card : theme.primary }]}
              onPress={() => setSelectedCat(null)}
            >
              <Text style={[styles.chipText, { color: selectedCat === null ? theme.textSecondary : '#fff' }]}>All Categories</Text>
            </TouchableOpacity>
            {cats.map(c => (
              <TouchableOpacity
                key={c.id}
                style={[styles.chip, { borderColor: selectedCat === c.id ? c.color : theme.border, backgroundColor: selectedCat === c.id ? c.color : theme.card }]}
                onPress={() => setSelectedCat(selectedCat === c.id ? null : c.id)}
              >
                <Text style={[styles.chipText, { color: selectedCat === c.id ? '#fff' : theme.textSecondary }]}>{c.name}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>
        ...
        <View style={styles.listContainer}>
          {filtered.length === 0 ? (
            <EmptyState icon="list-outline" title="No activities" subtitle="Tap + to add your first activity" />
          ) : (
            filtered.map(a => (
              <ActivityCard
                key={a.id}
                ...
                onPress={() => router.push(`/activity/${a.id}/edit?tripId=${id}` as any)}
              />
            ))
          )}
        </View>
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  header: { margin: 16, borderRadius: 16, borderWidth: 1, padding: 16, marginBottom: 0 },
  ...
  weatherRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 10, borderRadius: 8, padding: 8 },
  weatherText: { fontSize: 13, fontWeight: '600' },
  filterBar: { paddingTop: 12, paddingBottom: 4 },
  searchRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginHorizontal: 16, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 10, borderWidth: 1, marginBottom: 8 },
