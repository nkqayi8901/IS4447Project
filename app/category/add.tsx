...
export default function AddCategoryScreen() {
  ...
  const handleSave = async () => {
    ...
    if (!user) return;
    setLoading(true);
    await db.insert(categories).values({ userId: user.id, name: name.trim(), color: selectedColor, icon: selectedIcon });
    setLoading(false);
    router.back();
  };
      ...
      <ScrollView style={{ backgroundColor: theme.background }} contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.iconGrid}>
          {CATEGORY_ICONS.map(icon => (
            <TouchableOpacity
              key={icon}
              style={[styles.iconItem, { borderColor: selectedIcon === icon ? selectedColor : theme.border, backgroundColor: selectedIcon === icon ? selectedColor + '20' : theme.card }]}
        ...
        <TouchableOpacity style={[styles.btn, { backgroundColor: theme.primary }, loading && styles.btnDisabled]} onPress={handleSave} disabled={loading}>
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>Save Category</Text>}
        </TouchableOpacity>
      </ScrollView>
