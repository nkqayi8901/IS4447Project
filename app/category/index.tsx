...
export default function CategoriesScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      if (!user) return;
      db.select().from(categories).where(eq(categories.userId, user.id)).then(setCatList);
    }, [user])
  );

  const handleDelete = (id: number) => {
    Alert.alert('Delete Category', 'Are you sure? Activities using this category will lose their association.', [
        ...
        renderItem={({ item }) => (
          <View style={[styles.row, { backgroundColor: theme.card, borderColor: theme.border }]}>
            ...
            <View style={styles.actions}>
              <TouchableOpacity onPress={() => router.push(`/category/${item.id}/edit` as any)} style={[styles.iconBtn, { backgroundColor: theme.primary + '20' }]}>
                <Ionicons name="pencil-outline" size={16} color={theme.primary} />
              </TouchableOpacity>
              <TouchableOpacity onPress={() => handleDelete(item.id)} style={[styles.iconBtn, { backgroundColor: theme.danger + '20' }]}>
                <Ionicons name="trash-outline" size={16} color={theme.danger} />
              </TouchableOpacity>
            </View>
          </View>
        )}
        ListEmptyComponent={<EmptyState icon="albums-outline" title="No categories" subtitle="Tap + to create one" />}
        contentContainerStyle={[styles.list, catList.length === 0 && { flex: 1 }]}
...
const styles = StyleSheet.create({
  ...
  row: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderRadius: 12, borderWidth: 1, padding: 14, marginBottom: 10 },
