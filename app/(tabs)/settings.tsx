...
export default function SettingsScreen() {
  ...
  type RowProps = {
    ...
    onPress?: () => void; right?: React.ReactNode; destructive?: boolean;
  };

  const Row = ({ icon, iconColor, label, onPress, right, destructive }: RowProps) => {
    ...
    return (
      ...
      >
        ...
        {right ?? (onPress ? <Ionicons name="chevron-forward" size={14} color={theme.textSecondary} /> : null)}
      </TouchableOpacity>
    );
  };
  ...
  return (
    ...
    >
      {/* Profile Card */}
      <View style={[styles.profileCard, { backgroundColor: theme.card }, shadow.md]}>
      ...
      <View style={[styles.section, { backgroundColor: theme.card }, shadow.sm]}>
        <Row icon="moon" label="Dark Mode"
          right={<Switch value={isDark} onValueChange={toggleTheme} thumbColor="#fff" trackColor={{ false: theme.border, true: theme.primary }} />}
        />
        <Row icon="notifications" label="Daily Reminders"
          right={<Switch value={notificationsEnabled} onValueChange={handleToggleNotifications} thumbColor="#fff" trackColor={{ false: theme.border, true: theme.primary }} />}
        />
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.card }, shadow.sm]}>
        <Row icon="albums" label="Categories" onPress={() => router.push('/category/index' as any)} />
        <Row
          icon="sparkles"
          iconColor="#F59E0B"
          label={loadingDemo ? 'Loading demo data...' : 'Load Demo Data'}
          onPress={loadingDemo ? undefined : handleLoadDemo}
          right={loadingDemo ? <ActivityIndicator size="small" color={theme.primary} /> : undefined}
        />
      </View>
      ...
      <Text style={[styles.footer, { color: theme.textSecondary }]}>Spectacular Solomon's Trip Planner v1.0 · IS4447</Text>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  avatarText: { color: '#fff', fontFamily: 'Poppins_800ExtraBold', fontSize: 24 },
  email: { fontFamily: 'Poppins_400Regular', fontSize: 13, marginTop: 2 },
  sectionTitle: {
    ...text.label,
    marginLeft: spacing.lg, marginBottom: 8, marginTop: 4,
  },
  section: { marginHorizontal: spacing.lg, borderRadius: radius.xl, marginBottom: 20, overflow: 'hidden' },
  ...
  footer: { textAlign: 'center', fontFamily: 'Poppins_400Regular', fontSize: 12, marginVertical: 28 },
});
