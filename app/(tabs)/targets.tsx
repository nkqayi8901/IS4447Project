...
import { targets, activities, trips, categories } from '@/db/schema';
...
type TargetWithProgress = {
  id: number; name: string; period: string; metric: string;
  targetValue: number; current: number; categoryName: string | null; categoryColor: string | null;
};
        ...
        const enriched = await Promise.all(
          rows.map(async t => {
            ...
            for (const tripId of tripIds) {
              ...
              allActs.push(...acts.filter(a => a.date >= start && a.date <= end));
            }
            if (t.categoryId) {
              allActs = allActs.filter(a => a.categoryId === t.categoryId);
            }
            const current = t.metric === 'duration'
            ...
            return {
              id: t.id, name: t.name, period: t.period, metric: t.metric,
              targetValue: t.targetValue, current,
              categoryName: cat?.name ?? null, categoryColor: cat?.color ?? null,
            };
          })
        );
    ...
    return (
      ...
      >
        <View style={styles.cardHeader}>
          <View style={{ flex: 1 }}>
            <Text style={[styles.targetName, { color: theme.text }]}>{item.name}</Text>
          ...
          {exceeded ? (
            <View style={[styles.badge, { backgroundColor: theme.success + '20' }]}>
              ...
              <Text style={[styles.badgeText, { color: theme.success }]}>Done!</Text>
            </View>
          ) : null}
        </View>
        <ProgressBar progress={progress} color={item.categoryColor ?? theme.primary} />
        <View style={styles.labelRow}>
          <Text style={[styles.progressLabel, { color: theme.textSecondary }]}>{label}</Text>
          <Text style={[styles.pctLabel, { color: exceeded ? theme.success : theme.primary }]}>
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState icon="trophy-outline" title="No targets set" subtitle="Tap + to create a goal" />
        }
        contentContainerStyle={[styles.list, targetList.length === 0 && { flex: 1 }]}
