...
export default function AddTargetScreen() {
  ...
  useEffect(() => {
    if (!user) return;
    db.select().from(categories).where(eq(categories.userId, user.id)).then(setCats);
  }, [user]);

  const validate = () => {
    const e: Record<string, string> = {};
    if (!name.trim()) e.name = 'Target name is required';
    if (!targetValue || Number(targetValue) <= 0) e.value = 'Enter a valid goal value';
    setErrors(e);
  ...
  const handleSave = async () => {
    ...
    await db.insert(targets).values({
      userId: user.id, categoryId: selectedCat, name: name.trim(),
      period, metric, targetValue: Number(targetValue),
    });
    setLoading(false);
    router.back();
  };
    ...
    <View style={styles.chipSection}>
      ...
      <View style={styles.chipRow}>
        {options.map(o => (
          <TouchableOpacity
            key={o.key}
            style={[styles.chip, { borderColor: value === o.key ? theme.primary : theme.border, backgroundColor: value === o.key ? theme.primary : theme.card }]}
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={{ backgroundColor: theme.background }} contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        <FormField label="Target Name" placeholder="e.g. Weekly Activity Goal" value={name} onChangeText={setName} error={errors.name} />
        <FormField label={`Goal Value (${metric === 'duration' ? 'minutes' : 'activities'})`} placeholder="e.g. 300" value={targetValue} onChangeText={setTargetValue} keyboardType="numeric" error={errors.value} />
        ...
        <TouchableOpacity style={[styles.btn, { backgroundColor: theme.primary }, loading && styles.btnDisabled]} onPress={handleSave} disabled={loading}>
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>Create Target</Text>}
        </TouchableOpacity>
      </ScrollView>
